...
//         const token = localStorage.getItem("authToken");
//         const response = await axios.get(
//           `${process.env.REACT_APP_BASE_URL}/admin/borrow/get`,
//           {
//             headers: {
//               Authorization: token,
//               "Content-Type": "application/json",
...
import {
  ...
  const Borrow = () => {
    ...
    // DataGrid columns configuration - only view functionality
    const columns = [
      { field: "id", headerName: "Borrow ID", width: 220 },
      {
        field: "customerName",
        headerName: "Customer Name",
        flex: 1,
    ...
    // Fetch borrow data from the API
    const fetchBorrowData = async () => {
      try {
        ...
        // Check if response.data is an array or wrapped inside another object
        const borrowArray = Array.isArray(response.data)
          ? response.data
          : response.data?.data || [];
  
        const formattedBorrows = borrowArray.map((borrow) => {
          // Format the date
          ...
          // Extract customer name
          const customerName = borrow.userId 
            ? `${borrow.userId.firstName} ${borrow.userId.lastName}`
            : "N/A";
  
          // Extract book title
          const bookTitle = borrow.bookId?.title || "N/A";
  
          // Extract author name
          const authorName = borrow.bookId?.author 
            ? `${borrow.bookId.author.firstName} ${borrow.bookId.author.lastName}`
            : "N/A";
  
          // Extract deposit amount
          const depositeAmount = borrow.depositeAmount || 0;
  
          return {
            id: borrow._id,
            _id: borrow._id,
            customerName,
            customerEmail: borrow.userId?.email || "N/A",
            bookTitle,
            authorName,
            borrowDate,
            depositeAmount,
        ...
        setBorrowData(formattedBorrows);
      } catch (error) {
        console.log(error);
    ...
    return (
      <Box m="20px">
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Header title="BORROWINGS" subtitle="Manage Book Borrowing Information" />
        </Box>
        ...
        >
          ...
          <DialogContent>
            {selectedBorrow && (
              <Box sx={{ mt: 2 }}>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Borrow ID:</strong> {selectedBorrow.id}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Customer:</strong> {selectedBorrow.customerName}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Email:</strong> {selectedBorrow.customerEmail}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Book Borrowed:</strong> {selectedBorrow.bookTitle}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Author:</strong> {selectedBorrow.authorName}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Borrow Date:</strong> {selectedBorrow.borrowDate}
                </Typography>
                <Typography variant="subtitle1" sx={{ mb: 1 }}>
                  <strong>Deposit Amount:</strong> ${selectedBorrow.depositeAmount}
                </Typography>
              </Box>
            )}
          </DialogContent>
