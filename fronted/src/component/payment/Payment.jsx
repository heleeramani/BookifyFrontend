...
//   // Get user info from fetched data or use fallback
//   const getUserInfo = () => {
//     if (userData) {
//       return {
//         id: userData._id || 'USR-unknown',
...
import React, { useState, useEffect } from 'react';
import { jsPDF } from 'jspdf';
// import 'jspdf-autotable'; // Commented out since it's causing issues
import axios from 'axios';
import './Payment.css';
...
const Payment = ({ paymentDetails, onClose }) => {
  ...
  const downloadInvoice = () => {
    ...
    const cellPadding = 3;
    const rowHeight = 10;
    const colWidths = [50, 60, 15, 25, 25];
    ...
    let currentX = 15;
    const headerTexts = ["Item", "Description", "Qty", "Price", "Total"];
    ...
    headerTexts.forEach((text, i) => {
      ...
      // Add text
      doc.text(text, currentX + cellPadding, startY + rowHeight - cellPadding);
    ...
    // Get transaction items and create table rows
    const items = getTransactionItems();
    ...
    items.forEach((item, index) => {
      ...
      if (index % 2 === 0) {
        doc.setFillColor(240, 240, 240);
        doc.rect(15, currentY, colWidths.reduce((sum, width) => sum + width, 0), rowHeight, 'F');
      }
      
      // Add cell data
      currentX = 15;
      
      // Item title (first column)
      let title = item.title;
      if (title.length > 20) title = title.substring(0, 17) + '...';
      doc.text(title, currentX + cellPadding, currentY + rowHeight - cellPadding);
      currentX += colWidths[0];
      
      // Description (second column)
      let desc = item.description;
      if (desc.length > 25) desc = desc.substring(0, 22) + '...';
      doc.text(desc, currentX + cellPadding, currentY + rowHeight - cellPadding);
      currentX += colWidths[1];
      
      // Quantity (third column)
      doc.text(item.quantity.toString(), currentX + cellPadding, currentY + rowHeight - cellPadding);
      currentX += colWidths[2];
      
      // Price (fourth column)
      doc.text(`₹${item.price.toFixed(2)}`, currentX + cellPadding, currentY + rowHeight - cellPadding);
      currentX += colWidths[3];
      
      // Total (fifth column)
      doc.text(`₹${(item.price * item.quantity).toFixed(2)}`, currentX + cellPadding, currentY + rowHeight - cellPadding);
